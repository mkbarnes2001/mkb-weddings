import { AdminActionButton, AdminActionLabel, AdminActionLink, AdminActionRouterLink, AdminActionSummary } from "../components/ui/AdminActionControl";
...
function headingFontFamily(_value: string) {
  return '"Montserrat", "Avenir Next", Avenir, "Helvetica Neue", Arial, sans-serif';
}
  ...
  return (
    <div className="client-gallery-editor-page p-4 sm:p-6 lg:p-8" style={{ maxWidth: 1680 }}>
      <style>{`
        ...
        .admin-shell .client-gallery-photo-toolbar {
          display: flex;
          flex-wrap: wrap;
          align-items: center;
          gap: 8px;
        }
        .admin-shell .client-gallery-photo-toolbar__library { width: 32px; flex: 0 0 32px; }
        .admin-shell .client-gallery-photo-toolbar__sort { width: 160px; flex: 0 1 160px; }
        .admin-shell .client-gallery-photo-toolbar__search { width: auto; flex: 1 1 180px; }
      `}</style>
      <AdminPageHeader
      ...
      <div className="client-gallery-editor-shell mt-4">
        ...
        <div className="client-gallery-editor-main" style={{ minWidth: 0 }}>
          ...
          {activeTab === "photos" ? <section className="mt-5">
            <div className="rounded-2xl border border-black/10 bg-white p-4">
              ...
              <div className="client-gallery-photo-toolbar">
                ...
                {contentToolsEnabled ? <details className="client-gallery-photo-toolbar__library" style={{ position: "relative" }}>
                  ...
                  <div className="rounded-xl border border-black/10 bg-white p-3 shadow-lg" style={{ position: "absolute", left: 0, top: "calc(100% + 8px)", width: "min(560px, calc(100vw - 120px))", zIndex: 60 }}>
                    <div className="flex gap-2"><input value={assetSearch} onChange={(e) => setAssetSearch(e.target.value)} placeholder="Search filename, caption or alt…" className="min-w-0 flex-1 rounded-lg border border-black/15 px-3 py-2 text-sm" /><AdminActionButton onClick={searchAssets} disabled={busy} className="rounded-lg border border-black px-4 py-2 text-sm">Search</AdminActionButton></div>
              ...
              {selectedAssets.size ? <div className="mt-4 rounded-xl border border-black/10 bg-neutral-50 p-3 flex items-center justify-between gap-3 flex-wrap"><strong className="text-sm">{selectedAssets.size} selected</strong><div className="flex items-center gap-2 flex-wrap"><select value={bulkAlbumId} onChange={(e) => setBulkAlbumId(e.target.value)} className="rounded-lg border border-black/15 bg-white px-3 py-2 text-sm"><option value="">Choose album…</option>{activeAlbums.map((album) => <option key={album.id} value={album.id}>{album.name}</option>)}</select><AdminActionButton disabled={!bulkAlbumId || busy} onClick={addSelectedToAlbum} className="rounded-lg border border-black/15 bg-white px-3 py-2 text-sm inline-flex items-center gap-2 disabled:opacity-40"><FolderPlus className="h-4 w-4" /> Add to album</AdminActionButton>{activeAlbumId ? <AdminActionButton disabled={busy} onClick={removeSelectedFromAlbum} className="rounded-lg border border-black/15 bg-white px-3 py-2 text-sm">Remove from this album</AdminActionButton> : null}<AdminActionButton aria-label="Clear selection" onClick={() => setSelectedAssets(new Set())} className="rounded-lg p-2"><X className="h-4 w-4" /></AdminActionButton></div></div> : null}

              {uploads.length ? <div className="mt-4 rounded-xl border border-black/10 p-3"><div className="space-y-2">{uploads.map((item) => <div key={item.id} className="rounded-lg bg-neutral-50 p-3" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 80px 36px", gap: 10, alignItems: "center" }}><div className="min-w-0"><div className="flex items-center gap-2">{item.status === "done" ? <CheckCircle2 className="h-4 w-4 text-green-700" /> : item.status === "error" ? <AlertCircle className="h-4 w-4 text-red-700" /> : <UploadCloud className="h-4 w-4 text-neutral-500" />}<span className="text-xs truncate">{item.file.name}</span></div><div className="mt-2" style={{ height: 4, borderRadius: 99, background: "#ddd" }}><div style={{ width: `${item.progress}%`, height: "100%", background: item.status === "error" ? "#b91c1c" : "#111", borderRadius: 99 }} /></div><p className="mt-1 text-[10px] text-neutral-500">{item.error || item.stage}</p></div><span className="text-xs text-right">{item.progress}%</span>{item.status === "error" ? <AdminActionButton aria-label="Retry upload" onClick={() => updateUpload(item.id, { status: "queued", progress: 0, stage: "Ready", error: "" })}><RefreshCw className="h-4 w-4" /></AdminActionButton> : <AdminActionButton aria-label={`Remove ${item.file.name} from upload queue`} disabled={item.status === "uploading" || uploading} onClick={() => setUploads((current) => current.filter((upload) => upload.id !== item.id))}><X className="h-4 w-4" /></AdminActionButton>}</div>)}</div><div className="mt-3 flex justify-end"><AdminActionButton disabled={uploading || queuedCount === 0} onClick={uploadQueuedOriginals} className="rounded-lg bg-black text-white px-4 py-2 text-sm disabled:opacity-40">{uploading ? "Uploading…" : `Upload ${queuedCount} original${queuedCount === 1 ? "" : "s"}`}</AdminActionButton></div></div> : null}
                    ...
                    <div style={{ aspectRatio: "4/3", position: "relative", background: "#eee", overflow: "visible", borderRadius: 9 }}>
                      ...
                      <div data-photo-menu style={{ position: "absolute", right: 7, bottom: 7 }}>
                        ...
                        {openPhotoMenuId === asset.assetId ? <div className="rounded-xl border border-black/10 bg-white p-1 shadow-xl" style={{ position: "absolute", right: 0, bottom: "calc(100% + 6px)", width: 220, zIndex: 50 }}>
                          <AdminActionButton type="button" onClick={() => { setPreviewAssetId(asset.assetId); setOpenPhotoMenuId(""); }} className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 hover:bg-neutral-50"><Eye className="h-4 w-4" /> View photo</AdminActionButton>
                          {asset.hasOriginal ? <AdminActionLink href={AdminApiService.clientGalleryOriginalDownloadUrl(id, asset.assetId)} className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 hover:bg-neutral-50"><Download className="h-4 w-4" /> Download original</AdminActionLink> : <span className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 text-neutral-400"><Download className="h-4 w-4" /> Original unavailable</span>}
                          <AdminActionButton type="button" disabled={gallery.coverAssetId === asset.assetId} onClick={() => runPhotoAction({ action: "setCover", assetId: asset.assetId }, "Cover updated.")} className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 hover:bg-neutral-50 disabled:text-neutral-400"><Star className="h-4 w-4" /> {gallery.coverAssetId === asset.assetId ? "Current cover" : "Set as gallery cover"}</AdminActionButton>
                          <AdminActionButton type="button" onClick={() => runPhotoAction({ action: "setHidden", assetId: asset.assetId, hidden: !asset.hidden }, asset.hidden ? "Image shown." : "Image hidden.")} className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 hover:bg-neutral-50">{asset.hidden ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />} {asset.hidden ? "Show in client gallery" : "Hide from client gallery"}</AdminActionButton>
                          {activeAlbums.length ? <div className="mt-1 border-t border-black/5 p-2"><label className="text-[10px] uppercase tracking-[.1em] text-neutral-400">Add to album</label><div className="mt-1 flex gap-1"><select value={photoMenuAlbumId} onChange={(event) => setPhotoMenuAlbumId(event.target.value)} className="min-w-0 flex-1 rounded-lg border border-black/15 bg-white px-2 py-1.5 text-xs"><option value="">Choose…</option>{activeAlbums.map((album) => <option key={album.id} value={album.id}>{album.name}</option>)}</select><AdminActionButton aria-label="Create album" type="button" disabled={!photoMenuAlbumId} onClick={() => addPhotoToAlbum(asset.assetId)} className="rounded-lg border border-black/15 p-2 disabled:opacity-30"><FolderPlus className="h-3.5 w-3.5" /></AdminActionButton></div></div> : null}
                          <div className="mt-1 border-t border-black/5 pt-1"><AdminActionButton type="button" onClick={() => { if (window.confirm(`Remove ${asset.filename} from this client gallery? The canonical Asset Library image and private original will be preserved.`)) runPhotoAction({ action: "remove", assetId: asset.assetId }, "Image removed."); }} className="w-full rounded-lg px-3 py-2 text-left text-sm inline-flex items-center gap-2 text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /> Remove from gallery</AdminActionButton></div>
                        </div> : null}
                      </div>
                    </div>
                  </div>
            ...
            {activeAlbum ? <div className="mt-5 text-right"><AdminActionButton onClick={() => mutateAlbum({ action: "archive", albumId: activeAlbum.id }, "Album archived.").then(() => setActiveAlbumId(""))} className="text-xs text-red-700 underline underline-offset-4">Archive this album</AdminActionButton></div> : null}
          </section> : null}
            ...
            <section id="favourites-panel" className="rounded-2xl border border-black/10 bg-white p-4 sm:p-5">
              <div className="client-gallery-panel-header">
                ...
                <div className="client-gallery-panel-actions">
                  <AdminActionRouterLink to={`/admin/client-galleries/${id}/review?source=favourites&group=combined`} className="rounded-lg border border-black/15 px-3 py-2 text-xs inline-flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" /> View</AdminActionRouterLink>
                  {gallery.favouriteCount > 0 ? <AdminActionLink href={AdminApiService.clientGalleryBulkDownloadUrl(id, { source: "favourites", group: "combined" })} className="rounded-lg bg-black text-white px-3 py-2 text-xs inline-flex items-center gap-1.5"><Download className="h-3.5 w-3.5" /> Download all</AdminActionLink> : null}
                </div>
              </div>
            </section>
              ...
              <div className="client-gallery-selection-stack">
                <aside className="client-gallery-selection-create">
                  ...
                  <div className="client-gallery-selection-create-grid">
                    ...
                    <AdminActionButton disabled={busy || !selectionDraft.name.trim()} onClick={createSelectionRequest} className="rounded-lg bg-black px-4 py-2 text-xs text-white disabled:opacity-40">Create request</AdminActionButton>
                  </div>
                </aside>
                  ...
                  {detail.selectionRequests.length ? detail.selectionRequests.map((request) => {
                    ...
                    return <div key={request.id} className="rounded-xl border border-black/10 p-3.5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {request.status === "active" ? <AdminActionButton onClick={() => mutateSelection({ action: "archiveRequest", requestId: request.id }, "Selection request archived.")} title="Archive" className="rounded-md p-1.5 text-neutral-500 hover:bg-neutral-100"><Trash2 className="h-3.5 w-3.5" /></AdminActionButton> : null}
                      </div>
                      {responses.map((selection) => <div key={selection.id} className="client-gallery-response-row mt-2.5 rounded-lg bg-neutral-50 p-2.5">
                        ...
                        <div className="client-gallery-response-actions">
                          <AdminActionRouterLink title="View thumbnails" to={`/admin/client-galleries/${id}/review?source=selection&selectionId=${encodeURIComponent(selection.id)}`} className="rounded-md border border-black/10 p-1.5"><Eye className="h-3.5 w-3.5" /></AdminActionRouterLink>
                          {selection.assets.length ? <AdminActionLink title="Download originals" href={AdminApiService.clientGalleryBulkDownloadUrl(id, { source: "selection", selectionId: selection.id })} className="rounded-md border border-black/10 p-1.5"><Download className="h-3.5 w-3.5" /></AdminActionLink> : null}
                          <AdminActionButton title="Copy filenames" disabled={!selection.assets.length} onClick={() => copySelectionFilenames(selection.assets.map((asset) => asset.filename))} className="rounded-md border border-black/10 p-1.5 disabled:opacity-30"><Copy className="h-3.5 w-3.5" /></AdminActionButton>
                          <AdminActionButton title="Download CSV" disabled={!selection.assets.length} onClick={() => downloadSelectionCsv(selection)} className="rounded-md border border-black/10 p-1.5 disabled:opacity-30"><Download className="h-3.5 w-3.5" /></AdminActionButton>
                          {selection.status === "submitted" ? <AdminActionButton title="Reopen" onClick={() => mutateSelection({ action: "reopenSelection", selectionId: selection.id }, "Selection reopened for editing.")} className="rounded-md border border-black/10 p-1.5"><RotateCcw className="h-3.5 w-3.5" /></AdminActionButton> : null}
                        </div>
                      </div>)}
                    </div>;
          ...
          {activeTab === "access" ? <section className="client-gallery-settings-column">
            <div className="client-gallery-access-panel rounded-2xl border border-black/10 bg-white">
              ...
              <div className="client-gallery-access-list">
                ...
                <AdminActionButton onClick={save} disabled={busy} className="client-gallery-access-save rounded-lg bg-black text-white inline-flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            <aside className="client-gallery-authorised-panel rounded-2xl border border-black/10 bg-white">
              ...
              <div className="mt-3 space-y-2">
                {detail.contacts.map((contact) => <div key={contact.emailNormalized} className="client-gallery-contact-card">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <AdminActionButton className="client-gallery-contact-remove" title="Remove" aria-label={`Remove ${contact.displayName || contact.email}`} onClick={() => mutateContact({ action: "remove", email: contact.email }, "Access contact removed.")}>
                      <X />
                    </AdminActionButton>
                  </div>
                </div>)}
              ...
              <div className="mt-4 rounded-xl bg-neutral-50 p-3 space-y-2">
                ...
                <AdminActionButton disabled={busy || !contactDraft.email.trim()} onClick={addContact} className="w-full rounded-lg border border-black/15 bg-white px-3 py-2 text-[10px] inline-flex items-center justify-center gap-2 disabled:opacity-40">
          ...
          {activeTab === "store" && printStoreEnabled ? <section className="client-gallery-settings-column mt-5">
            <div className="rounded-2xl border border-black/10 bg-white p-5">
              ...
              {!storeDraft || !storeData ? <p className="mt-5 text-sm text-neutral-500">Loading Print Store settings…</p> : <>
                ...
                <div className="mt-5 flex items-center gap-3"><AdminActionButton type="button" onClick={saveStoreSettings} disabled={busy || (storeDraft.enabled && !storeDraft.priceListId)} className="rounded-lg bg-black px-5 py-3 text-white inline-flex items-center gap-2 disabled:opacity-40"><Save className="h-4 w-4" /> Save Print Store</AdminActionButton><AdminActionRouterLink to="/admin/print-store" className="rounded-lg border border-black/15 px-5 py-3 inline-flex items-center gap-2"><ShoppingBag className="h-4 w-4" /> Manage catalogue</AdminActionRouterLink></div>
              </>}
            </div>
          ...
          {activeTab === "settings" ? <section className="client-gallery-settings-column mt-5"><div className="rounded-2xl border border-black/10 bg-white p-5"><div className="flex items-center gap-2"><Settings className="h-5 w-5" /><h3 className="text-xl font-semibold">Gallery settings</h3></div><p className="mt-2 text-sm text-neutral-600">General gallery identity and wedding linkage. Access and security now live in the Access tab.</p><div className="client-gallery-form-grid mt-5"><label className="block"><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Title</span><input value={draft.title || ""} onChange={(e) => setDraft({ ...draft, title: e.target.value })} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5" /></label><label className="block"><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Status</span><select value={draft.status || "draft"} onChange={(e) => setDraft({ ...draft, status: e.target.value as any })} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5 bg-white"><option value="draft">Draft</option><option value="live">Live</option><option value="archived">Archived</option></select></label><label className="block" style={{ gridColumn: "1 / -1" }}><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Custom gallery slug</span><input value={draft.slug || ""} onChange={(e) => setDraft({ ...draft, slug: e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") })} placeholder="ian-and-shannen" className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5" /><span className="mt-1 block text-[10px] text-neutral-500">Used as the clean public gallery URL. Existing token-based links continue to work.</span></label><label className="block"><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Client name</span><input value={draft.clientName || ""} onChange={(e) => setDraft({ ...draft, clientName: e.target.value })} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5" /></label><label className="block"><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Primary client email</span><input value={draft.clientEmail || ""} onChange={(e) => setDraft({ ...draft, clientEmail: e.target.value })} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5" /></label><label className="block" style={{ gridColumn: "1 / -1" }}><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Wedding</span><select value={draft.weddingSlug || ""} onChange={(e) => setDraft({ ...draft, weddingSlug: e.target.value })} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5 bg-white"><option value="">No linked wedding</option>{detail.weddings.map((wedding) => <option key={wedding.slug} value={wedding.slug}>{wedding.title || wedding.couple || wedding.slug}</option>)}</select></label><label className="block" style={{ gridColumn: "1 / -1" }}><span className="text-xs uppercase tracking-[.12em] text-neutral-500">Gallery introduction</span><textarea value={draft.intro || ""} onChange={(e) => setDraft({ ...draft, intro: e.target.value })} rows={5} className="mt-1 w-full rounded-lg border border-black/15 px-3 py-2.5" /></label></div><div className="mt-5 flex items-center gap-3"><AdminActionButton onClick={save} disabled={busy} className="rounded-lg bg-black text-white px-5 py-3 inline-flex items-center gap-2 disabled:opacity-50"><Save className="h-4 w-4" /> Save settings</AdminActionButton>{gallery.status === "live" ? <AdminActionButton onClick={() => navigator.clipboard?.writeText(shareUrl)} className="rounded-lg border border-black/15 px-5 py-3 inline-flex items-center gap-2"><Copy className="h-4 w-4" /> Copy gallery link</AdminActionButton> : null}</div></div></section> : null}
        </div>
      </div>
      {previewAsset ? <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.88)", display: "grid", placeItems: "center", padding: 24 }} onMouseDown={() => setPreviewAssetId("")}><div style={{ maxWidth: "92vw", maxHeight: "90vh", position: "relative" }} onMouseDown={(event) => event.stopPropagation()}><img src={previewAsset.webSrc || previewAsset.thumbSrc} alt={previewAsset.filename} style={{ maxWidth: "92vw", maxHeight: "84vh", objectFit: "contain", display: "block" }} /><div className="mt-3 flex items-center justify-between gap-3 text-white"><span className="text-sm">{previewAsset.filename}</span><div className="flex gap-2">{previewAsset.hasOriginal ? <AdminActionLink href={AdminApiService.clientGalleryOriginalDownloadUrl(id, previewAsset.assetId)} className="rounded-lg bg-white text-black px-3 py-2 text-sm inline-flex items-center gap-2"><Download className="h-4 w-4" /> Download original</AdminActionLink> : null}<AdminActionButton type="button" onClick={() => setPreviewAssetId("")} className="rounded-lg border border-white/30 px-3 py-2 text-sm">Close</AdminActionButton></div></div></div></div> : null}
    </div>
  );
}
