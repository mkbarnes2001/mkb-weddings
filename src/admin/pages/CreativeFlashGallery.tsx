...
export function CreativeFlashGallery() {
  ...
  return <div className="space-y-6">
    ...
    <section style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 320px",gap:"20px",alignItems:"start"}}>
      ...
      <aside style={{position:'sticky',top:112,borderRadius:24,border:'1px solid rgba(0,0,0,.12)',background:'#fff',padding:16,maxHeight:'calc(100vh - 128px)',overflowY:'auto'}}>{!activeImage?<p className="text-sm text-neutral-500">Select an image to edit its gallery settings.</p>:<div className="space-y-4"><SafeImage image={activeImage} className="max-h-[240px] w-full rounded-2xl object-contain bg-neutral-100"/><div><p className="break-all text-xs text-neutral-500">{activeImage.filename}</p><p className="mt-1 text-sm font-medium">{activeImage.venueName}</p></div>
