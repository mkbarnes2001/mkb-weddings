...
export function GalleryByVenue() {
        ...
        if (!cancelled) {
          setLoadError(
            error instanceof Error
              ? error.message
              : "Unable to load venue galleries.",
          );
        }
      })
      .finally(() => {
  ...
  const venueCards = useMemo(() => {
    ...
    return [...venues].sort((a, b) => {
      const aRank =
        pinned.get(a.name.toLowerCase()) ??
        Number.MAX_SAFE_INTEGER;
      const bRank =
        pinned.get(b.name.toLowerCase()) ??
        Number.MAX_SAFE_INTEGER;
            ...
            {venueCards.map((venue) => (
              ...
              >
                <ImageWithFallback
                  ...
                  alt={`${venue.name} wedding photography venue gallery`}
