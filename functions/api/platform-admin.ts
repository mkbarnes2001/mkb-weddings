...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (actor.platformRole !== "platform_admin" || !actor.permissions.includes("platform:admin") || actor.accessMode === "support") {
      return Response.json({ error: "WedPlanned platform administrator access is required." }, { status: 403 });
    }

    if (context.request.method === "GET") {
      const platformAdmin = await getPlatformAdministration(context.env.MKB_DB, actor);
      return Response.json({ ok: true, platformAdmin, auth: actor }, { headers: { "Cache-Control": "private, no-store" } });
    }
    ...
    if (action === "saveModuleConfiguration") {
      platformAdmin = await updatePlatformModuleConfiguration(context.env.MKB_DB, actor, body?.module || body);
    } else if (action === "saveSupplierTaxonomy") {
      platformAdmin = await updatePlatformSupplierTaxonomy(context.env.MKB_DB, actor, body);
    } else {
      return Response.json({ error: "Unsupported platform administration action." }, { status: 400 });
    }
    return Response.json({ ok: true, platformAdmin, auth: actor }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
