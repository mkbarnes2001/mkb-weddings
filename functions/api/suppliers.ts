...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    if (context.request.method === "GET") {
      ...
      if (url.searchParams.get("view") === "master") {
        return Response.json({
          ok: true,
          suppliers: await listMasterSuppliers(context.env.MKB_DB, true),
        });
      }
      return Response.json({
        ok: true,
    ...
    if (context.request.method === "POST") {
      ...
      const supplier = await createMasterSupplier(context.env.MKB_DB, payload?.supplier || payload);
      return Response.json({ ok: true, supplier }, { status: 201 });
    }

    if (context.request.method === "PUT") {
      const payload = await context.request.json();
      const supplier = await updateMasterSupplier(context.env.MKB_DB, payload?.supplier || payload);
    ...
    if (context.request.method === "DELETE") {
      ...
      const supplier = await archiveMasterSupplier(context.env.MKB_DB, id);
      return Response.json({ ok: true, supplier });
    }
