...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    async function resolve(id: string) {
      if (!id) return null;
      const image: any = await env.MKB_DB.prepare(`SELECT asset_key, image_id, thumb_src, full_src, alt FROM images WHERE workspace_id = ? AND (asset_key = ? OR image_id = ?) LIMIT 1`).bind(workspaceId, id, id).first();
