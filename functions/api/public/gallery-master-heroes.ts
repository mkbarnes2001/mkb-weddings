...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  try {
    ...
    const settings = { venueHeroImageId: "", momentsHeroImageId: "", ...parse(row?.document_json, {}) };
    ...
    const [venue, moments] = await Promise.all([resolve(settings.venueHeroImageId), resolve(settings.momentsHeroImageId)]);
    return Response.json({ ok: true, venue, moments }, { headers: { "Cache-Control": "public, max-age=60" } });
