...
require("getCrmContractTemplates(" not in crm, "contract library loading must stay on its guarded standalone page")
contract_page = read("src/admin/pages/CRMContractTemplates.tsx")
require("if (!canManage || creating || !name.trim()) return;" in contract_page, "standalone contract creation is not permission guarded")
...
require('if (bookingsEnabled && templateType === "quotes")' in templates and "getCrmQuoteTemplates" in templates and "getCrmQuoteCatalogue" in templates, "quote template/catalogue APIs are not bookings-conditional")
require("visibleTemplateOptions(auth.permissions, enabledEntitlementKeys)" in read("src/admin/pages/CRMTemplates.tsx"), "Templates hub does not filter libraries by capability")
require('bookingsEnabled && view === "quotes"' in templates, "Quote template actions are not bookings conditional")

require('key: "templates", label: "Templates", settingsOnly: true, to: "/admin/crm/templates", icon: FileText, match:' in modules, "Templates nav remains bookings-only")
require('key: "commercial-settings", label: "Commercial settings", settingsOnly: true, to: "/admin/crm?view=commercial-settings", icon: Settings, requiredEntitlements: ["bookings"]' in modules, "Commercial settings nav is not bookings-core")
...
db.close()
require(str(version) == "54", f"mixed surfaces gate changed schema: {version}")
require(not list((ROOT / "d1/migrations").glob("055_*.sql")), "mixed surfaces gate must not add migration 055")
...
print("  Templates hub filters each standalone library by entitlement: verified")
